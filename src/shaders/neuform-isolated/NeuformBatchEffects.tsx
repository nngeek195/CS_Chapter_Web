...
(function () {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (event.data.type === 'pointer-move') {
      try {
    ...
    if (event.data.type === 'pointer-leave') {
      try {
        document.dispatchEvent(new Event('mouseleave'));
      } catch (err) {}
      return;
    }
    if (event.data.type !== 'threeui-controls') return;
...
(function () {
  ...
  function isolate() {
    ...
    requestAnimationFrame(function () {
      ...
      try { window.parent.postMessage({ type: 'threeui-ready' }, '*'); } catch (e) {}
    });
  }
