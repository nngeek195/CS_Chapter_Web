...
function pt(s, v, k) {
  ...
  const rt = () => {
    const n = k(), o = n.phrase || "IEEE CS CHAPTER | SUSL / ", r = mt(122), p = '"ThreeUI Fragment Mono", ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace', u = Math.max(m, 1.5), I = Math.hypot(Math.max(S * 0.55, S * 0.48), Math.max(w * 0.52, w * 0.5)) + 40;
    D = [];
    ...
    p !== H && (H = p, rt()), i.clearRect(0, 0, S, w);
    const bg = o.background ? o.background : (r ? "#eef1f6" : "#06131d");
    if (bg !== "transparent") {
      i.fillStyle = bg;
      i.fillRect(0, 0, S, w);
    }
